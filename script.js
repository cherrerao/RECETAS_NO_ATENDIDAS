...
const filtrarProductosDebounced = debounce(() => {
    ...
    sugerenciasDiv.innerHTML = '';
    resultadosFinales.forEach(med => {
        const item = document.createElement('div');
        item.className = 'sugerencia-item';
        item.dataset.codigo = med.codigo || '';
        item.dataset.descripcion = med.descripcion || '';
        item.addEventListener('click', () => seleccionarProducto(item.dataset.codigo, item.dataset.descripcion));

        const codigoDiv = document.createElement('div');
        codigoDiv.className = 'sugerencia-codigo';
        codigoDiv.textContent = `[${med.codigo || ''}]`;

        const descripcionDiv = document.createElement('div');
        descripcionDiv.className = 'sugerencia-descripcion';
        descripcionDiv.textContent = med.descripcion || '';

        const categoriaDiv = document.createElement('div');
        categoriaDiv.className = 'sugerencia-categoria';
        categoriaDiv.style.fontSize = '0.85em';
        categoriaDiv.style.color = '#999';
        categoriaDiv.style.marginTop = '2px';
        categoriaDiv.textContent = med.categoria || '';

        item.appendChild(codigoDiv);
